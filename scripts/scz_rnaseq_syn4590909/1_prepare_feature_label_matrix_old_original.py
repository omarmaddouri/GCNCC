...
flags.DEFINE_string('gene_expression', 'RNAseq_SCZ_BD_GVEX_datExpr.csv', 'Gene expression file.')
flags.DEFINE_string('clinical_info', 'RNAseq_SCZ_BD_GVEX_datMeta.csv', 'Clinical status file.')
...
clinical_info = pd.read_csv("{}/data/raw_input/{}/{}".format(project_path, FLAGS.dataset, FLAGS.clinical_info), usecols=(0,11)).to_numpy(dtype=np.dtype(str))
sample_labels = {}
for i in range(clinical_info.shape[0]):
    sample_labels[clinical_info[i,0]] = clinical_info[i,1]

with open("{}/data/raw_input/{}/{}".format(project_path, FLAGS.dataset, FLAGS.gene_expression), encoding="utf-8") as lines:
    clinical_status = np.genfromtxt(itertools.islice(lines, 0, 1), delimiter=",", dtype=np.dtype('U25'))
...
GE = pd.read_csv("{}/data/raw_input/{}/{}".format(project_path, FLAGS.dataset, FLAGS.gene_expression), skiprows=1, usecols=tuple(selected_columns)).to_numpy(dtype=np.dtype(str))
probes = GE[:,0]
